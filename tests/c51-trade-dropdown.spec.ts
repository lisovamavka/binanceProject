...
test('C51 - Trade dropdown', async ({ page }) => {
    ...
    const allOptions = [
        ...homePage.header.getBasicTradeOptions(),
        ...homePage.header.getAdvancedTradeOptions(),
    ];

    for (const option of allOptions) {
        const optionLocator = await homePage.header.getTradeOption(option);
        await expect(optionLocator).toBeVisible();
        await homePage.header.tradeDropdownOptionsTrialClick(optionLocator);
    }
    ...
    for (const option of allOptions) {
        const optionLocator = await homePage.header.getTradeOption(option);
        await expect(optionLocator).toBeVisible();
        const optionCard = await homePage.header.getTradeOptionCard(option);
    ...
    await homePage.header.trade.hover();
    await homePage.header.tradeDropdown.waitFor({ state: 'visible' });
    const spotLocator = await homePage.header.getTradeOption('Spot');
    await expect(spotLocator).toBeVisible();
    await homePage.header.clickSpotAndNavigate();
